...
/**
 * DataSource를 통한 DB 연결 테스트
 */
@ExtendWith(SpringExtension.class)
@ContextConfiguration(classes = AppCtx.class)    // 테스트 환경에서 스프링 컨테이너가 생성됨
//@Transactional
public class JdbcTemplateTest {
    ...
    @Transactional
    void insertTest() {
        // DataAccessException - RuntimeException - 예외처리 없어도 실행
    ...
    void selectTest() {
        String sql = "SELECT * FROM MEMBER ORDER BY USER_NO" ;
        // 람다식의 메서드 참조 이용
    ...
    @Test
    @DisplayName("단일 조회 테스트")    // 예외 발생 가능성이 높음 : 결과 데이터가 1개가 아니면 에러
    void selectTest2() {
